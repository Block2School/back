...
async def get_profile(r: Request, credentials: str = Depends(JWTChecker())):
...
async def update_profile(r: Request, profile_model: ProfileModel, credentials: str = Depends(JWTChecker())):
    jwt = JWT.decodeJWT(credentials)
    Log.route_log(r, "user routes", jwt["uuid"])
...
async def get_user_profile(r: Request, username: str, credentials: str = Depends(JWTChecker())):
    jwt = JWT.decodeJWT(credentials)
    Log.route_log(r, "user routes", jwt["uuid"])
...
async def add_qrcode_authenticator(r: Request, wordlist_model: WordlistModel, credentials: str = Depends(JWTChecker())):
    jwt = JWT.decodeJWT(credentials)
    Log.route_log(r, "user routes", jwt["uuid"])
...
async def remove_qrcode_authenticator(r: Request, wordlist_model: WordlistModel, credentials: str= Depends(JWTChecker())):
    jwt = JWT.decodeJWT(credentials)
    Log.route_log(r, "user routes", jwt["uuid"])
    if wordlist_model.wordlist == None:
...
async def add_discord_authenticator(r: Request, discord_authenticator_model: AddDiscordAuthenticatorModel, credentials: str = Depends(JWTChecker())):
    jwt = JWT.decodeJWT(credentials)
    Log.route_log(r, "user routes", jwt["uuid"])
...
async def remove_discord_authenticator(r: Request, wordlist_model: NeedWordListModel, credentials: str = Depends(JWTChecker())):
    jwt = JWT.decodeJWT(credentials)
    Log.route_log(r, "user routes", jwt["uuid"])
    if wordlist_model.wordlist != None:
...
async def get_friends(r: Request, credentials: str = Depends(JWTChecker())):
    jwt = JWT.decodeJWT(credentials)
    Log.route_log(r, "user routes", jwt["uuid"])
...
async def add_friend(r: Request, friend: FriendsModel, credentials: str = Depends(JWTChecker())):
    jwt = JWT.decodeJWT(credentials)
    Log.route_log(r, "user routes", jwt["uuid"])
...
async def remove_friend(r: Request, friend: FriendsModel, credentials: str = Depends(JWTChecker())):
    jwt = JWT.decodeJWT(credentials)
    Log.route_log(r, "user routes", jwt["uuid"])
    if friend.friend_uuid != None:
...
async def search_user(r: Request, user: str = None, page: int = 1, offset: int = 50, credentials: str = Depends(JWTChecker())):
    jwt = JWT.decodeJWT(credentials)
    Log.route_log(r, "user routes", jwt["uuid"])
    if not user:
