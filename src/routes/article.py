...
async def get_all_articles(r: Request) -> JSONResponse:
    """
    Récupération de tous les articles
    """
    Log.route_log(r, "article routes", "open_route")
    articles = ArticleService.get_all_articles()
    return JSONResponse({'data': articles}, status_code=200)

@router.get('/id/{id}', tags=['article'], responses=get_article_response)
async def get_article(r: Request, id: int) -> JSONResponse:
    """
...
async def create_article(r: Request, article: ArticleModel, jwt: str = Depends(AdminChecker(1))) -> JSONResponse:
    """
...
async def update_article(r: Request, article: ArticleModel, jwt: str = Depends(AdminChecker(1))) -> JSONResponse:
    """
...
async def delete_article(r: Request, id: IdModel, jwt: str = Depends(AdminChecker(1))) -> JSONResponse:
    """
    ...
    if id.id != None:
        ...
        if result:
            return JSONResponse({'success': 'Article deleted !'}, status_code=200)
        else:
